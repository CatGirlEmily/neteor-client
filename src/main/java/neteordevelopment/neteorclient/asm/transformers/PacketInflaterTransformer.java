...
package neteordevelopment.neteorclient.asm.transformers;

import neteordevelopment.neteorclient.asm.AsmTransformer;
import neteordevelopment.neteorclient.asm.Descriptor;
import neteordevelopment.neteorclient.asm.MethodInfo;
...
public class PacketInflaterTransformer extends AsmTransformer {
    ...
    public void transform(ClassNode klass) {
        ...
        if (method == null) error("[Neteor Client] Could not find method PacketInflater.decode()");
        ...
        for (AbstractInsnNode insn : method.instructions) {
            if (insn instanceof TypeInsnNode typeInsn && typeInsn.getOpcode() == Opcodes.NEW && typeInsn.desc.equals("io/netty/handler/codec/DecoderException")) {
                ...
                if (newCount == 2) {
                    ...
                    list.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "neteordevelopment/neteorclient/systems/modules/Modules", "get", "()Lneteordevelopment/neteorclient/systems/modules/Modules;", false));
                    list.add(new LdcInsnNode(Type.getType(AntiPacketKick.class)));
                    list.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL, "neteordevelopment/neteorclient/systems/modules/Modules", "isActive", "(Ljava/lang/Class;)Z", false));
        ...
        error("[Neteor Client] Failed to modify PacketInflater.decode()");
    }
}
