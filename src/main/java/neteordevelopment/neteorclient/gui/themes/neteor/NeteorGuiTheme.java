...
import neteordevelopment.neteorclient.gui.DefaultSettingsWidgetFactory;
...
import neteordevelopment.neteorclient.gui.themes.neteor.widgets.*;
import neteordevelopment.neteorclient.gui.themes.neteor.widgets.input.WNeteorDropdown;
import neteordevelopment.neteorclient.gui.themes.neteor.widgets.input.WNeteorSlider;
import neteordevelopment.neteorclient.gui.themes.neteor.widgets.input.WNeteorTextBox;
import neteordevelopment.neteorclient.gui.themes.neteor.widgets.pressable.*;
import neteordevelopment.neteorclient.gui.utils.AlignmentX;
import neteordevelopment.neteorclient.gui.utils.CharFilter;
import neteordevelopment.neteorclient.gui.widgets.*;
...
import neteordevelopment.neteorclient.gui.widgets.input.WSlider;
import neteordevelopment.neteorclient.gui.widgets.input.WTextBox;
import neteordevelopment.neteorclient.gui.widgets.pressable.*;
...
public class NeteorGuiTheme extends GuiTheme {
    ...
    public WWindow window(WWidget icon, String title) {
        return w(new WNeteorWindow(icon, title));
    }

    @Override
    public WLabel label(String text, boolean title, double maxWidth) {
        if (maxWidth == 0 && !text.contains("\n")) return w(new WNeteorLabel(text, title));
        return w(new WNeteorMultiLabel(text, title, maxWidth));
    }

    @Override
    public WHorizontalSeparator horizontalSeparator(String text) {
        return w(new WNeteorHorizontalSeparator(text));
    }

    @Override
    public WVerticalSeparator verticalSeparator() {
        return w(new WNeteorVerticalSeparator());
    }

    @Override
    protected WButton button(String text, GuiTexture texture) {
        return w(new WNeteorButton(text, texture));
    }

    @Override
    protected WConfirmedButton confirmedButton(String text, String confirmText, GuiTexture texture) {
        return w(new WNeteorConfirmedButton(text, confirmText, texture));
    }

    @Override
    public WMinus minus() {
        return w(new WNeteorMinus());
    }

    @Override
    public WConfirmedMinus confirmedMinus() {
        return w(new WNeteorConfirmedMinus());
    }

    @Override
    public WPlus plus() {
        return w(new WNeteorPlus());
    }

    @Override
    public WCheckbox checkbox(boolean checked) {
        return w(new WNeteorCheckbox(checked));
    }

    @Override
    public WSlider slider(double value, double min, double max) {
        return w(new WNeteorSlider(value, min, max));
    }

    @Override
    public WTextBox textBox(String text, String placeholder, CharFilter filter, Class<? extends WTextBox.Renderer> renderer) {
        return w(new WNeteorTextBox(text, placeholder, filter, renderer));
    }

    @Override
    public <T> WDropdown<T> dropdown(T[] values, T value) {
        return w(new WNeteorDropdown<>(values, value));
    }

    @Override
    public WTriangle triangle() {
        return w(new WNeteorTriangle());
    }

    @Override
    public WTooltip tooltip(String text) {
        return w(new WNeteorTooltip(text));
    }

    @Override
    public WView view() {
        return w(new WNeteorView());
    }

    @Override
    public WSection section(String title, boolean expanded, WWidget headerWidget) {
        return w(new WNeteorSection(title, expanded, headerWidget));
    }

    @Override
    public WAccount account(WidgetScreen screen, Account<?> account) {
        return w(new WNeteorAccount(screen, account));
    }

    @Override
    public WWidget module(Module module, String title) {
        return w(new WNeteorModule(module, title));
    }

    @Override
    public WQuad quad(Color color) {
        return w(new WNeteorQuad(color));
    }

    @Override
    public WTopBar topBar() {
        return w(new WNeteorTopBar());
    }

    @Override
    public WFavorite favorite(boolean checked) {
        return w(new WNeteorFavorite(checked));
    }
