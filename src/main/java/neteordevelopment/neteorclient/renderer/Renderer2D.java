...
import neteordevelopment.neteorclient.gui.renderer.packer.TextureRegion;
...
public class Renderer2D {
    ...
    public Renderer2D(boolean textured) {
        ...
        triangles = new MeshBuilder(textured ? NeteorRenderPipelines.UI_TEXTURED : NeteorRenderPipelines.UI_COLORED);
        lines = new MeshBuilder(NeteorRenderPipelines.UI_COLORED_LINES);
    }
    ...
    public void render(String samplerName, GpuTextureView samplerView, GpuSampler sampler) {
        ...
        MeshRenderer.begin()
            ...
            .pipeline(NeteorRenderPipelines.UI_COLORED_LINES)
            .mesh(lines)
            .end();

        MeshRenderer.begin()
            .attachments(MinecraftClient.getInstance().getFramebuffer())
            .pipeline(textured ? NeteorRenderPipelines.UI_TEXTURED : NeteorRenderPipelines.UI_COLORED)
            .mesh(triangles)
