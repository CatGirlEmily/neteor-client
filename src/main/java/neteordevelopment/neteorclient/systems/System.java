...
public abstract class System<T> implements ISerializable<T> {
    ...
    public System(String name) {
        ...
        if (name != null) {
            this.file = new File(NeteorClient.FOLDER, name + ".nbt");
    ...
    public void save(File folder) {
        ...
        try {
            File tempFile = File.createTempFile(NeteorClient.MOD_ID, file.getName());
            ...
            NeteorClient.LOG.error("Error saving {}. Possibly corrupted?", this.name, e);
        }
    }
        ...
        try {
            ...
            if (file.exists()) {
                try {
                    ...
                    NeteorClient.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
                    NeteorClient.LOG.info("Saved settings backup to '{}'.", backup);
                }
            }
        } catch (IOException e) {
            NeteorClient.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
