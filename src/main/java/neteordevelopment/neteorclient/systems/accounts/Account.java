...
import neteordevelopment.neteorclient.mixin.FileCacheAccessor;
import neteordevelopment.neteorclient.mixin.MinecraftClientAccessor;
import neteordevelopment.neteorclient.mixin.PlayerSkinProviderAccessor;
...
public abstract class Account<T extends Account<?>> implements ISerializable<T> {
    ...
    public static void setSession(Session session) {
        ...
        mca.neteor$setUserApiService(apiService);
        mca.neteor$setSocialInteractionsManager(new SocialInteractionsManager(mc, apiService));
        mca.neteor$setProfileKeys(ProfileKeys.create(apiService, session, mc.runDirectory.toPath()));
        mca.neteor$setAbuseReportContext(AbuseReportContext.create(ReporterEnvironment.ofIntegratedServer(), apiService));
        mca.neteor$setGameProfileFuture(CompletableFuture.supplyAsync(() -> mc.getApiServices().sessionService().fetchProfile(mc.getSession().getUuidOrNull(), true), Util.getIoWorkerExecutor()));
    }
    ...
    public static void applyLoginEnvironment(YggdrasilAuthenticationService authService) {
        ...
        PlayerSkinProvider.FileCache skinCache = ((PlayerSkinProviderAccessor) mc.getSkinProvider()).neteor$getSkinCache();
        Path skinCachePath = ((FileCacheAccessor) skinCache).neteor$getDirectory();
        mca.neteor$setApiServices(ApiServices.create(authService, mc.runDirectory));
        mca.neteor$setSkinProvider(new PlayerSkinProvider(skinCachePath, mc.getApiServices(), new PlayerSkinTextureDownloader(mc.getNetworkProxy(), mc.getTextureManager(), mc), mc));
    }
