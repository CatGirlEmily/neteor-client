...
package neteordevelopment.neteorclient.systems.hud;

import neteordevelopment.neteorclient.events.neteor.CustomFontChangedEvent;
...
import neteordevelopment.neteorclient.systems.hud.elements.*;
...
public class Hud extends System<Hud> implements Iterable<HudElement> {
    public static final HudGroup GROUP = new HudGroup("Neteor");
    ...
    private final Setting<Boolean> hideInMenus = sgGeneral.add(new BoolSetting.Builder()
        .name("hide-in-menus")
        .description("Hides the neteor hud when in inventory screens or game menus.")
        .defaultValue(false)
        .build()
    );
    ...
    private void resetToDefaultElementsImpl() {
        ...
        // Top Left
        add(NeteorTextHud.WATERMARK, 4, 4, XAnchor.Left, YAnchor.Top);
        add(NeteorTextHud.FPS, 4, 4 + h, XAnchor.Left, YAnchor.Top);
        add(NeteorTextHud.TPS, 4, 4 + h * 2, XAnchor.Left, YAnchor.Top);
        add(NeteorTextHud.PING, 4, 4 + h * 3, XAnchor.Left, YAnchor.Top);
        add(NeteorTextHud.SPEED, 4, 4 + h * 4, XAnchor.Left, YAnchor.Top);

        // Top Right
        add(ActiveModulesHud.INFO, -4, 4, XAnchor.Right, YAnchor.Top);

        // Bottom Right
        add(NeteorTextHud.POSITION, -4, -4, XAnchor.Right, YAnchor.Bottom);
        add(NeteorTextHud.OPPOSITE_POSITION, -4, -4 - h, XAnchor.Right, YAnchor.Bottom);
        add(NeteorTextHud.ROTATION, -4, -4 - h * 2, XAnchor.Right, YAnchor.Bottom);
    }
