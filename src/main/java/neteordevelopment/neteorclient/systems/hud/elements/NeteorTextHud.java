...
package neteordevelopment.neteorclient.systems.hud.elements;

import neteordevelopment.neteorclient.systems.hud.Hud;
import neteordevelopment.neteorclient.systems.hud.HudElementInfo;

public class NeteorTextHud {
    public static final HudElementInfo<TextHud> INFO = new HudElementInfo<>(Hud.GROUP, "text", "Displays arbitrary text with Starscript.", NeteorTextHud::create);
    ...
    static {
        ...
        MODULE_ENABLED = addPreset("Module enabled", "Kill Aura: {neteor.is_module_active(\"kill-aura\") ? #2 \"ON\" : #3 \"OFF\"}", 0);
        MODULE_ENABLED_WITH_INFO = addPreset("Module enabled with info", "Kill Aura: {neteor.is_module_active(\"kill-aura\") ? #2 \"ON\" : #3 \"OFF\"} #1{neteor.get_module_info(\"kill-aura\")}", 0);
        WATERMARK = addPreset("Watermark", "{neteor.name} #1{neteor.version}");
