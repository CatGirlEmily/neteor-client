...
package neteordevelopment.neteorclient.systems.macros;
...
public class Macros extends System<Macros> implements Iterable<Macro> {
    ...
    public void add(Macro macro) {
        macros.add(macro);
        NeteorClient.EVENT_BUS.subscribe(macro);
        save();
    }
    ...
    public void remove(Macro macro) {
        if (macros.remove(macro)) {
            NeteorClient.EVENT_BUS.unsubscribe(macro);
            save();
        }
    }
    ...
    public Macros fromTag(NbtCompound tag) {
        for (Macro macro : macros) NeteorClient.EVENT_BUS.unsubscribe(macro);

        macros = NbtUtils.listFromTag(tag.getListOrEmpty("macros"), Macro::new);

        for (Macro macro : macros) NeteorClient.EVENT_BUS.subscribe(macro);
        return this;
    }
}
