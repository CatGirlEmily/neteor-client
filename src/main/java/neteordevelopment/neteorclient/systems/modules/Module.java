...
public abstract class Module implements ISerializable<Module>, Comparable<Module> {
    ...
    public Module(Category category, String name, String description, String... aliases) {
        if (name.contains(" ")) NeteorClient.LOG.warn("Module '{}' contains invalid characters in its name making it incompatible with Neteor Client commands.", name);
        ...
        for (NeteorAddon addon : AddonManager.ADDONS) {
    ...
    public void toggle() {
        if (!active) {
            ...
            if (runInMainMenu || Utils.canUpdate()) {
                if (autoSubscribe) NeteorClient.EVENT_BUS.subscribe(this);
                onActivate();
            }
        }
        else {
            if (runInMainMenu || Utils.canUpdate()) {
                if (autoSubscribe) NeteorClient.EVENT_BUS.unsubscribe(this);
                onDeactivate();
            }
