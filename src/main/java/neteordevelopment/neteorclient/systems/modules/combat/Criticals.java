...
public class Criticals extends Module {
    ...
    private void onSendPacket(PacketEvent.Send event) {
        if (event.packet instanceof IPlayerInteractEntityC2SPacket packet && packet.neteor$getType() == PlayerInteractEntityC2SPacket.InteractType.ATTACK) {
            if (mace.get() && mc.player.getMainHandStack().getItem() instanceof MaceItem) {
                ...
                Entity entity = packet.neteor$getEntity();
                    ...
                    case Jump, MiniJump -> {
                        if (!sendPackets) {
                            ...
                            if (mode.get() == Mode.Jump) {
                                ...
                                ((IVec3d) mc.player.getVelocity()).neteor$setY(0.25);
                                sendTimer = 4;
                            }
                            event.cancel();
                        }
    ...
    private void sendPacket(double height) {
        ...
        ((IPlayerMoveC2SPacket) packet).neteor$setTag(1337);
