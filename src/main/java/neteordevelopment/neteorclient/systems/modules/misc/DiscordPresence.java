...
import neteordevelopment.neteorclient.gui.utils.StarscriptTextBoxRenderer;
...
public class DiscordPresence extends Module {
    ...
    private final Setting<List<String>> line2Strings = sgLine2.add(new StringListSetting.Builder()
        ...
        .defaultValue("Neteor on Crack!", "{round(server.tps, 1)} TPS", "Playing on {server.difficulty} difficulty.", "{server.player_count} Players online")
    ...
    public DiscordPresence() {
        super(Categories.Misc, "discord-presence", "Displays Neteor as your presence on Discord.");
    ...
    public void onActivate() {
        ...
        String largeText = "%s %s".formatted(NeteorClient.NAME, NeteorClient.VERSION);
        if (!NeteorClient.BUILD_NUMBER.isEmpty()) largeText += " Build: " + NeteorClient.BUILD_NUMBER;
        rpc.setLargeImage("neteor_client", largeText);
    ...
    private void recompile(List<String> messages, List<Script> scripts) {
        ...
        for (String message : messages) {
            Script script = NeteorStarscript.compile(message);
        ...
        if (Utils.canUpdate()) {
            ...
            if (line1Ticks >= line1UpdateDelay.get() || forceUpdate) {
                if (!line1Scripts.isEmpty()) {
                    ...
                    String message = NeteorStarscript.run(line1Scripts.get(i));
            ...
            if (line2Ticks >= line2UpdateDelay.get() || forceUpdate) {
                if (!line2Scripts.isEmpty()) {
                    ...
                    String message = NeteorStarscript.run(line2Scripts.get(i));
            ...
            if (!lastWasInMainMenu) {
                rpc.setDetails(NeteorClient.NAME + " " + (NeteorClient.BUILD_NUMBER.isEmpty() ? NeteorClient.VERSION : NeteorClient.VERSION + " " + NeteorClient.BUILD_NUMBER));
                ...
                else if (mc.currentScreen instanceof WidgetScreen) rpc.setState("Browsing Neteor's GUI");
    ...
    public WWidget getWidget(GuiTheme theme) {
        ...
        help.action = () -> Util.getOperatingSystem().open("https://github.com/NeteorDevelopment/neteor-client/wiki/Starscript");
