...
import neteordevelopment.neteorclient.events.entity.player.InteractEntityEvent;
...
import neteordevelopment.neteorclient.mixin.HandledScreenAccessor;
...
public class InventoryTweaks extends Module {
    ...
    private boolean sort() {
        ...
        Slot focusedSlot = ((HandledScreenAccessor) screen).neteor$getFocusedSlot();
        ...
        for (int i = start; i < end; i++) {
            ...
            if (sleep > 0) {
                try {
                    ...
                    NeteorClient.LOG.error("Error when sleeping the slot mover", e);
                }
            }
    ...
    public void steal(ScreenHandler handler) {
        NeteorExecutor.execute(() -> moveSlots(handler, 0, SlotUtils.indexToId(SlotUtils.MAIN_START), true));
    }

    public void dump(ScreenHandler handler) {
        int playerInvOffset = SlotUtils.indexToId(SlotUtils.MAIN_START);
        NeteorExecutor.execute(() -> moveSlots(handler, playerInvOffset, playerInvOffset + 4 * 9, false));
    }
