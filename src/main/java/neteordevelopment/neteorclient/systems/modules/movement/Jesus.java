...
import neteordevelopment.neteorclient.events.entity.player.CanWalkOnFluidEvent;
import neteordevelopment.neteorclient.events.packets.PacketEvent;
import neteordevelopment.neteorclient.events.world.CollisionShapeEvent;
import neteordevelopment.neteorclient.events.world.TickEvent;
import neteordevelopment.neteorclient.mixin.LivingEntityAccessor;
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if ((waterMode.get() == Mode.Bob && mc.player.isTouchingWater()) || (lavaMode.get() == Mode.Bob && mc.player.isInLava())) {
            ...
            if (mc.player.isTouchingWater() && fluidHeight > swimHeight) {
                ((LivingEntityAccessor) mc.player).neteor$swimUpwards(FluidTags.WATER);
            } else if (mc.player.isOnGround() && fluidHeight <= swimHeight && ((LivingEntityAccessor) mc.player).neteor$getJumpCooldown() == 0) {
                mc.player.jump();
                ((LivingEntityAccessor) mc.player).neteor$setJumpCooldown(10);
            } else {
                ((LivingEntityAccessor) mc.player).neteor$swimUpwards(FluidTags.LAVA);
            }
        }
        ...
        if (bubbleColumn) {
            if (mc.options.jumpKey.isPressed() && mc.player.getVelocity().getY() < 0.11) ((IVec3d) mc.player.getVelocity()).neteor$setY(0.11);
            return;
        }

        // Move up
        if (mc.player.isTouchingWater() || mc.player.isInLava()) {
            ((IVec3d) mc.player.getVelocity()).neteor$setY(0.11);
            tickTimer = 0;
            return;
        }
