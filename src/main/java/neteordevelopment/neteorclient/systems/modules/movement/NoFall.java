...
import neteordevelopment.neteorclient.mixin.PlayerMoveC2SPacketAccessor;
import neteordevelopment.neteorclient.mixininterface.IPlayerMoveC2SPacket;
...
public class NoFall extends Module {
    ...
    private void onSendPacket(PacketEvent.Send event) {
        ...
        if (mc.player.getAbilities().creativeMode
            ...
            || ((IPlayerMoveC2SPacket) event.packet).neteor$getTag() == 1337) return;


        if (!Modules.get().isActive(Flight.class)) {
            if (mc.player.isGliding()) return;
            if (mc.player.getVelocity().y > -0.5) return;
            ((PlayerMoveC2SPacketAccessor) event.packet).neteor$setOnGround(true);
        } else {
            ((PlayerMoveC2SPacketAccessor) event.packet).neteor$setOnGround(true);
        }
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (mode.get() == Mode.AirPlace) {
            ...
            Rotations.rotate(mc.player.getYaw(), 90, Integer.MAX_VALUE, () -> {
                ...
                ((IVec3d) mc.player.getVelocity()).neteor$setY(preY);
            });
        }
