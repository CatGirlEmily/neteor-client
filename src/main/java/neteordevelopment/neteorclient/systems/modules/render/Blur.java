...
import neteordevelopment.neteorclient.renderer.FixedUniformStorage;
...
public class Blur extends Module {
    ...
    private final Setting<Boolean> neteor = sgScreens.add(new BoolSetting.Builder()
        .name("neteor")
        .description("Applies blur to Neteor screens.")
        .defaultValue(true)
        .build());
    ...
    public Blur() {
        ...
        NeteorClient.EVENT_BUS.subscribe(new ConsumerListener<>(ResolutionChangedEvent.class, event -> {
            // Resize all fbos
            for (int i = 0; i < fbos.length; i++) {
                if (fbos[i] != null) {
                    fbos[i].close();
        ...
        NeteorClient.EVENT_BUS.subscribe(new ConsumerListener<>(RenderAfterWorldEvent.class, event -> onRenderAfterWorld()));
    }
    ...
    private void onRenderAfterWorld() {
        ...
        // Initial downsample
        renderToFbo(fbos[0], mc.getFramebuffer().getColorAttachmentView(), NeteorRenderPipelines.BLUR_DOWN, ubos[0]);

        // Downsample
        for (int i = 0; i < iterations; i++) {
            renderToFbo(fbos[i + 1], fbos[i], NeteorRenderPipelines.BLUR_DOWN, ubos[i + 1]);
        }

        // Upsample
        for (int i = iterations; i >= 1; i--) {
            renderToFbo(fbos[i - 1], fbos[i], NeteorRenderPipelines.BLUR_UP, ubos[i - 1]);
        }

        // Render output
        MeshRenderer.begin()
            .attachments(mc.getFramebuffer())
            .pipeline(NeteorRenderPipelines.BLUR_PASSTHROUGH)
            .fullscreen()
    ...
    private boolean shouldRender() {
        ...
        if (screen instanceof WidgetScreen) return neteor.get();
    ...
    private static final FixedUniformStorage<BlurUniformData> UNIFORM_STORAGE = new FixedUniformStorage<>("Neteor - Blur UBO", UNIFORM_SIZE, 6);
