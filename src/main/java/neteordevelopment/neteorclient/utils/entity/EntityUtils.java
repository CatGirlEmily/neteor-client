...
package neteordevelopment.neteorclient.utils.entity;
...
public class EntityUtils {
    ...
    public static BlockPos getCityBlock(PlayerEntity player) {
        ...
        for (Direction direction : DirectionAccessor.neteor$getHorizontal()) {
    ...
    public static boolean intersectsWithEntity(Box box, Predicate<Entity> predicate) {
        EntityLookup<Entity> entityLookup = ((WorldAccessor) mc.world).neteor$getEntityLookup();
        ...
        if (entityLookup instanceof SimpleEntityLookup<Entity> simpleEntityLookup) {
            SectionedEntityCache<Entity> cache = ((SimpleEntityLookupAccessor) simpleEntityLookup).neteor$getCache();
            LongSortedSet trackedPositions = ((SectionedEntityCacheAccessor) cache).neteor$getTrackedPositions();
            Long2ObjectMap<EntityTrackingSection<Entity>> trackingSections = ((SectionedEntityCacheAccessor) cache).neteor$getTrackingSections();
                ...
                while (longIterator.hasNext()) {
                    ...
                    if (s >= j && s <= m && t >= k && t <= n) {
                        ...
                        if (entityTrackingSection != null && entityTrackingSection.getStatus().shouldTrack()) {
                            for (Entity entity : ((EntityTrackingSectionAccessor) entityTrackingSection).<Entity>neteor$getCollection()) {
