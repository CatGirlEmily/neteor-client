...
public class Capes {
    private static final String CAPE_OWNERS_URL = "https://neteorclient.com/api/capeowners";
    private static final String CAPES_URL = "https://neteorclient.com/api/capes";
    ...
    @PreInit(dependencies = NeteorExecutor.class)
    public static void init() {
        ...
        NeteorExecutor.execute(() -> {
            ...
            Stream<String> lines = Http.get(CAPE_OWNERS_URL)
                .exceptionHandler(e -> NeteorClient.LOG.error("Could not load capes: {}",  e.getMessage()))
                .sendLines();
            if (lines != null) {
                lines.forEach(s -> {
        ...
        NeteorClient.EVENT_BUS.subscribe(Capes.class);
    }
    ...
    private static class Cape {
        ...
        public Cape(String name) {
            this.identifier = NeteorClient.identifier("capes/" + COUNT++);
            this.name = name;
        }
        ...
        public void download() {
            ...
            NeteorExecutor.execute(() -> {
                try {
                    ...
                    NeteorClient.LOG.error("Failed to download cape '{}'", name, e);
                }
            });
        }
