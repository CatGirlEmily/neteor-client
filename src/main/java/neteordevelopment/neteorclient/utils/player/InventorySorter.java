...
import neteordevelopment.neteorclient.mixininterface.ISlot;
...
public class InventorySorter {
    ...
    private void generateActions() {
        ...
        for (Slot slot : screen.getScreenHandler().slots) {
            if (getInvPart(slot) == originInvPart) slots.add(new MySlot(((ISlot) slot).neteor$getId(), slot.getStack()));
        }
    ...
    private InvPart getInvPart(Slot slot) {
        int i = ((ISlot) slot).neteor$getIndex();

        if (slot.inventory instanceof PlayerInventory && (!(screen instanceof CreativeInventoryScreen) || ((ISlot) slot).neteor$getId() > 8)) {
