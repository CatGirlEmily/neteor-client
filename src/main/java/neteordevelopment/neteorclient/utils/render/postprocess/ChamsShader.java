...
public class ChamsShader extends EntityShader {
    ...
    public ChamsShader() {
        super(NeteorRenderPipelines.POST_IMAGE);
        NeteorClient.EVENT_BUS.subscribe(ChamsShader.class);
    }
    ...
    public static void load() {
        try {
            ...
            for (String fileFormat : FILE_FORMATS) {
                Optional<Resource> optional = mc.getResourceManager().getResource(NeteorClient.identifier("textures/chams." + fileFormat));
            ...
            NeteorClient.LOG.error("Error loading the chams shader", e);
        }
    }
    ...
    private static final DynamicUniformStorage<UniformData> UNIFORM_STORAGE = new DynamicUniformStorage<>("Neteor - Image UBO", UNIFORM_SIZE, 16);
